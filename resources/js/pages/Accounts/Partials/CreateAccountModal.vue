...
import { X, File, Landmark, Palette, Check } from 'lucide-vue-next';
                ...
                <div class="p-6 space-y-6">
                    <div class="border-b-2 border-gray-500 pb-2">
                        <div class="flex items-baseline gap-2">
                            ...
                            <input type="text" :value="displayValue" @input="handleCurrencyInput" placeholder="0.00"
                    ...
                    <div class="flex items-center gap-4 border-b border-gray-700 py-3">
                        ...
                        <input type="text" v-model="form.name"
                            class="w-full bg-transparent outline-none placeholder-gray-400" placeholder="Description" />
                    </div>
                    <InputError :message="form.errors.name" />
                    <div class="flex items-center gap-4 border-b border-gray-700 py-3">
                        <Landmark class="h-5 w-5 text-gray-400" />
                        <select v-model="form.type" class="w-full bg-transparent outline-none appearance-none">
                            <option v-for="(label, key) in accountTypes" :key="key" :value="key" class="text-black">{{
                                label }}</option>
                        </select>
                    </div>
                    ...
                    <div class="flex justify-between items-center pt-4">
                        <label for="include_in_dashboard" class="text-sm cursor-pointer">Include sum on
                            dashboard</label>
                        <Switch id="include_in_dashboard" :checked="form.include_in_dashboard"
                            @update:checked="form.include_in_dashboard = $event" />
                    </div>
